/*--------------------------------------------------------------------------------------*/
/*  start.js  -  Description: This modal will open from the StartRun page and will keep */
/*  track of key statistics of the user's run. The user will be able to pause/play on   */
/*  command. On end, the review details page will be called and formatted data will be  */
/*  delivered                                                                           */
/*--------------------------------------------------------------------------------------*/
/*  Author: Sharan P. & Haran R.                                                        */
/*  Date: July 12, 2020                                                                 */
/*--------------------------------------------------------------------------------------*/
/*  Input: Latitide and longitude of the users current location                         */
/*  Output: Time (minutes), distance (km), average and current speed (min/km)           */
/*--------------------------------------------------------------------------------------*/

//import external packages
import React, { useState, useEffect } from "react";
import * as Location from "expo-location";
import { Text, View, Modal, TouchableWithoutFeedback, Keyboard, TouchableOpacity } from "react-native";
...
//default function
export default function Start({ setStartModal, addRun, navigation }) {
  //declare state
    ...
    if (isActive) {
      (async () => {
        ...
        if (lat1 != null && lon1 != null) {
          //calculate distance
          let distanceTraveled = distanceInKmBetweenEarthCoordinates(lat1, lon1, lat2, lon2);
          ...
          if (runPath.length > 5) {
            ...
            let currentDistanceTraveled =
              distanceInKmBetweenEarthCoordinates(runPath[c - 5].latitude, runPath[c - 5].longitude, runPath[c].latitude, runPath[c].longitude) *
              1000;
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={{ ...globalStyles.containerSecondary, ...{ backgroundColor: containerColor } }}>
        ...
        <View style={globalStyles.topLine}>
          <View style={globalStyles.speedData}>
            <View style={globalStyles.rightLine}>
              <View style={globalStyles.timeAndDistData}>
                <Text style={globalStyles.midSubTextLight}>{currentSpeed}</Text>
                <Text style={globalStyles.smallSubTextLight}>Current Speed</Text>
              </View>
            </View>
            <View style={globalStyles.timeAndDistData}>
              <Text style={globalStyles.midSubTextLight}>{avgSpeed}</Text>
              <Text style={globalStyles.smallSubTextLight}>Average Speed</Text>
            </View>
          </View>
        </View>
        <View style={globalStyles.horizontalSpaceAround}>
          <TouchableOpacity onPress={handlePlayPause}>
            <FontAwesome name={playPause} size={56} color={globalDesign.light} />
          </TouchableOpacity>
          <TouchableOpacity disabled={runPath.length == 0} onPress={navigateToReview}>
            <FontAwesome name="stop" size={56} color={globalDesign.light} />
          </TouchableOpacity>
        </View>
